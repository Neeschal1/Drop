...
export const normalizeProduct = (p) => {
  ...
  const price = typeof p.price === "number" ? `€${p.price}` : (p.price?.startsWith("€") ? p.price : `€${p.price || 0}`);
  const availableSizes = p.available_sizes || p.availableSizes || ["XS", "S", "M", "L", "XL"];
...
export const getProducts = async (gender = null) => {
  try {
    ...
    console.warn("Could not fetch products from API, using fallback data:", err);
  }

  // Fallback to local data
  const women = (Data[0]?.women || []).map((p) => normalizeProduct({ ...p, gender: "female" }));
  const men = (Data[0]?.men || []).map((p) => normalizeProduct({ ...p, gender: "male" }));
...
export const getProductById = async (productId) => {
  ...
  const match = all.find((p) => String(p.item) === String(productId) || String(p.id) === String(productId));
...
export const getCustomerReviews = async (productId = null) => {
  try {
    const url = productId ? `/review/product/${productId}/` : "/review/list-all-review/";
    ...
    if (rawReviews.length > 0) {
      return rawReviews.map((r) => ({
        ...
        date: r.review_timing ? new Date(r.review_timing).toLocaleDateString() : "Recent review",
        productName: r.product ? `Product #${r.product}` : "Verified Purchase",
      }));
    }
  } catch (err) {
    console.warn("Could not fetch reviews from API, using fallback reviews:", err);
  }
