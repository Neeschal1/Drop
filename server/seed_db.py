...
def seed():
    ...
    women_products = [
        ...
        },
        {
            ...
            "shipping_and_return": ["Free shipping over €100"]
        },
        {
        ...
        },
        {
            ...
            "availability": "In Stock",
            "available_sizes": ["XS", "S", "M", "L", "XL"],
            "details_and_care": ["Liquid drape satin finish", "Hand wash or gentle cycle"],
        ...
        },
        {
            ...
            "shipping_and_return": ["Complimentary shipping over €100"]
        },
        {
    ...
    for prod_data in all_products:
        existing = db.query(Product).filter(Product.itemname == prod_data["itemname"]).first()
        if not existing:
    ...
    sample_reviews = [
        {"customer_name": "Sophia Martin", "customer_rating": 5, "customer_review": "Absolutely love the fit and quality. The fabric feels premium and the pants look exactly like the pictures."},
        {"customer_name": "Daniel Carter", "customer_rating": 5, "customer_review": "The hoodie is incredibly comfortable and the oversized fit is perfect. Definitely ordering another one."},
        {"customer_name": "Emma Wilson", "customer_rating": 4, "customer_review": "Really nice jacket with a great silhouette. Shipping was quick and the packaging was also very neat."},
        {"customer_name": "Oliver James", "customer_rating": 5, "customer_review": "The quality exceeded my expectations. The material feels heavy and durable without being uncomfortable."},
        {"customer_name": "Mia Anderson", "customer_rating": 4, "customer_review": "The cargo pants have a really good fit and plenty of pocket space. The sizing was accurate for me."},
        {"customer_name": "James Wilson", "customer_rating": 5, "customer_review": "Simple, clean and comfortable. Exactly what I was looking for in an everyday wardrobe piece."},
        {"customer_name": "Ava Thompson", "customer_rating": 5, "customer_review": "The jeans fit beautifully and the washed finish looks even better in person. Highly recommended."},
        {"customer_name": "Lucas Brown", "customer_rating": 4, "customer_review": "Very comfortable and well made. I especially liked the attention to detail around the stitching."},
        {"customer_name": "Isabella Taylor", "customer_rating": 5, "customer_review": "Beautiful dress with a really elegant shape. It feels lightweight and comfortable while still looking premium."},
        {"customer_name": "Noah Williams", "customer_rating": 5, "customer_review": "The blazer has a very clean structure and fits perfectly. It works really well with both casual and formal outfits."},
        {"customer_name": "Grace Davis", "customer_rating": 4, "customer_review": "Really versatile skirt and easy to style. The material is comfortable and the fit is flattering."},
        {"customer_name": "Ethan Miller", "customer_rating": 5, "customer_review": "The shirt has a great oversized fit without looking messy. Fabric is lightweight and perfect for layering."}
    ]

    first_product_id = list(created_product_ids.values())[0] if created_product_ids else 1

    for idx, r in enumerate(sample_reviews):
        existing_rev = db.query(Review).filter(Review.customer_name == r["customer_name"]).first()
        if not existing_rev:
            p_id = list(created_product_ids.values())[idx % len(created_product_ids)] if created_product_ids else first_product_id
            rev = Review(
                product=p_id,
